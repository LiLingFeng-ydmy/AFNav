...
def fix_transformers_compatibility():
    try:
        ...
        original_init = PreTrainedTokenizer.__init__
        
        def patched_init(self, *args, **kwargs):
            original_init(self, *args, **kwargs)
            if not hasattr(self, 'unique_no_split_tokens'):
                self.unique_no_split_tokens = []
        
        PreTrainedTokenizer.__init__ = patched_init
        print("Applied transformers compatibility patch")
    except Exception as e:
        print(f"Transformers patch warning: {e}")
...
def run_exp(exp_name: str, exp_config: str, 
    ...
    for epid, metric in summary.items():
        for k, v in metric.items():
            if k == "sub_instruction_switches":
                detailed_switches[epid] = v  
            elif k == "locked_target_step":
                locked_target_steps[epid] = v
            elif k in summary_metrics:
    ...
    with open(config.CHECKPOINT_FOLDER + '/detailed_switches_val_unseen.json', 'w') as f:
        json.dump(detailed_switches, f, indent=2)
    with open(config.CHECKPOINT_FOLDER + '/locked_target_steps_val_unseen.json', 'w') as f:
        json.dump(locked_target_steps, f, indent=2)
