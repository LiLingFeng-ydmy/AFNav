...
class RxRVLNCEDatasetV1(Dataset):
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        llm_replys_path = getattr(config, 'LLM_REPLYS_PATH', None)
        if llm_replys_path:
            with open(llm_replys_path, 'r') as f:
                llm_replys = json.load(f)
        else:
            llm_replys = {}
        ...
        for role in self.extract_roles_from_config(config):
            with gzip.open(
                ...
                self.from_json(f.read(), scenes_dir=config.SCENES_DIR, llm_replys=llm_replys)
