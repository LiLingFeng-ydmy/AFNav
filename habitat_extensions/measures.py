"""
Implement all kinds of VLN measurements like SPL, nDTW, etc.
"""
    ...
    def update_metric(self, episode, *args: Any, **kwargs: Any):
        ...
        if len(self._metric['position']) > 0:
            if (current_position == self._metric['position'][-1]).all():
                return
        distance = self._sim.geodesic_distance(        # 计算当前位置到目标的测地距离|AB|
            current_position,                          # 智能体当前位置 A
            [goal.position for goal in episode.goals], # 目标位置 B
            episode,
        )
