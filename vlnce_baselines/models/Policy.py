...
class FusionMapPolicy(nn.Module):
    ...
    def _search_destination(self,                          
        ...
        for i, destination in enumerate(destinations): 
            ...
            destination_map = one_step_full_map[4 + map_idx] # 从 one_step_full_map 
    ...
    def forward(self, 
                ...
                backtrack_waypoint: np.ndarray = None,  
        ...
        x, y = x * (100 / self.resolution), y * (100 / self.resolution)
        position = np.array([y, x])
        if self.enable_backtrack and is_backtracking and backtrack_waypoint is not None:


            best_waypoint = backtrack_waypoint
            best_value = 0.0  
            sorted_waypoints = [backtrack_waypoint]
            sorted_values = [0.0]
        else:
            best_waypoint, best_value, sorted_waypoints, sorted_values = self.superpixel_policy(full_map, traversible,
                                                                                            value_map, collision_map,
                                                                                            detected_classes, position,
                                                                                            self.fmm_dist, replan,
        ...
        if search_destination: 
            ...
            if self.enable_backtrack and is_backtracking and backtrack_waypoint is not None:
                action = self._get_action(full_pose, backtrack_waypoint, full_map,
                ...
                destination_waypoint, score = self._search_destination(destination, classes, current_value, max_value,
                                                                   ...
                                                                   value_map, floor, traversible, current_detection,
                                                                   step)
