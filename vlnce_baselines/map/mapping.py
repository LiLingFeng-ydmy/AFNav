...
class Semantic_Mapping(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.detected_classes_for_mapping = OrderedSet()
        self.last_num_detected_classes = 0
    ...
    def sync_detected_classes(self, detected_classes: OrderedSet) -> None:

        self.detected_classes_for_mapping = detected_classes
        num_total_classes = len(detected_classes)

        if num_total_classes > self.last_num_detected_classes:
            self._dynamic_process(num_total_classes)
            self.last_num_detected_classes = num_total_classes
    ...
    def _visualize(self, 
        ...
        self.visited_vis[gx1:gx2, gy1:gy2] = vu.draw_line(last_start,                         
                                                          start,                              
        ...
        cx = int(pos[0]) + 670
        cy = int(pos[1]) + 50
        cv2.circle(self.vis_image, (cx, cy), 5, (0, 0, 255), -1)  # BGR红色，半径5
    ...
    def forward(self, obs: torch.Tensor, pose_obs: torch.Tensor):
        ...
        num_current_detected = c - 4
        num_total_detected = len(self.detected_classes_for_mapping)
        if num_total_detected > num_current_detected:
            pad_channels = num_total_detected - num_current_detected
            obs_pad = torch.zeros(bs, pad_channels, h, w).to(self.device)
            obs = torch.cat([obs, obs_pad], dim=1)
            c = obs.size(1)
            num_current_detected = num_total_detected
        num_detected_categories = max(num_current_detected, num_total_detected)
