    ...
    def _process_detections(self, detections: sv.Detections) -> sv.Detections:
        ...
        while i >= 0:
            if box_areas[i] / (self.width * self.height) < 0.95:
                ...
                mask_attr = getattr(detections, "mask", None)
            if mask_attr is None:
                mask_attr = getattr(detections, "mask_array", None)
            if mask_attr is not None:
                mask_attr = np.delete(mask_attr, i, axis=0)
                if hasattr(detections, "mask"):
                    detections.mask = mask_attr
                elif hasattr(detections, "mask_array"):
                    detections.mask_array = mask_attr
