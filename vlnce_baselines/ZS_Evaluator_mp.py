...
from modify.SoftResetBacktrack import BacktrackControllerV1 
from modify.DestinationSearchFRM import create_destination_search_frm  
import traceback
...
class ZeroShotVlnEvaluatorMP(BaseTrainer):
    def __init__(self, config: Config, segment_module=None, mapping_module=None) -> None:
        ...
        self.sub_instruction_switches = []  
        self.soft_backtrack_controller = BacktrackControllerV1(config) 
        self.enable_backtrack = config.EVAL.get('ENABLE_BACKTRACK', False)

        dest_frm_config = {
            'enabled': config.DESTINATION_SEARCH_FRM.ENABLED,
            'detection_mode': config.DESTINATION_SEARCH_FRM.DETECTION_MODE,
            'repeat_threshold': config.DESTINATION_SEARCH_FRM.REPEAT_THRESHOLD,
            'position_window': config.DESTINATION_SEARCH_FRM.POSITION_WINDOW,
            'position_precision': config.DESTINATION_SEARCH_FRM.POSITION_PRECISION,
            'base_radius': config.DESTINATION_SEARCH_FRM.BASE_RADIUS,
            'base_penalty': config.DESTINATION_SEARCH_FRM.BASE_PENALTY,
            'max_regions': config.DESTINATION_SEARCH_FRM.MAX_REGIONS,
            'merge_threshold_ratio': config.DESTINATION_SEARCH_FRM.MERGE_THRESHOLD_RATIO,
            'use_time_decay': config.DESTINATION_SEARCH_FRM.USE_TIME_DECAY,
            'decay_rate': config.DESTINATION_SEARCH_FRM.DECAY_RATE,
            'min_penalty': config.DESTINATION_SEARCH_FRM.MIN_PENALTY,
            'over_penalty_threshold': config.DESTINATION_SEARCH_FRM.OVER_PENALTY_THRESHOLD,
        }
        self.dest_frm = create_destination_search_frm(**dest_frm_config)
    ...
    def _calculate_metric(self, infos: List):
        ...
        metric['reached_last_subinstr'] = info.get('reached_last_subinstr', 0)
        metric['locked_target'] = info.get('locked_target', 0)
        metric['locked_target_step'] = info.get('locked_target_step', 0)
        metric['sub_instruction_switches'] = info.get('sub_instruction_switches', [])
    ...
    def _initialize_policy(self) -> None:
        ...
        self.segment_module = GroundedSAM(self.config, self.device) # GroundedSAM
        self.mapping_module = Semantic_Mapping(self.config.MAP).to(self.device) # Semantic_Mapping
        self.mapping_module.eval()
        
        self.value_map_module = ValueMap(self.config, self.mapping_module.map_shape, self.device) # model:Blip2ITM
        ...
        self.constraints_monitor = ConstraintsMonitor(self.config, self.device) # model:BlipVQA
    ...
    def _maps_initialization(self):
        ...
        self.mapping_module.sync_detected_classes(self.detected_classes)
    ...
    def _look_around(self):
        ...
        for step in range(0, 12):
            ...
            if dones[0]:
                return full_pose, obs, dones, infos, None, None
            batch_obs = self._batch_obs(obs)
            poses = torch.from_numpy(np.array([item['sensor_pose'] for item in obs])).float().to(self.device)
            self.mapping_module.sync_detected_classes(self.detected_classes)
        ...
        self._action = self.policy(self.value_map_module.value_map[1], self.collision_map,
                                    ...
                                    self.current_detections, self.current_episode_id, False, step, goal_position)
        
        return full_pose, obs, dones, infos, full_map, one_step_full_map
    ...
    def rollout(self):
        ...
        self.soft_backtrack_controller.reset()
        full_pose, obs, dones, infos, full_map, one_step_full_map = self._look_around()
        print("\n ========== START TO NAVIGATE ==========\n")
        ...
        if self.config.DESTINATION_SEARCH_FRM.ENABLED:
            self.dest_frm.reset()
        ...
        for step in range(12, self.max_step):
            ...
            if int(current_idx) >= len(self.sub_instructions) - 1:
                ...
                self.reached_last_subinstruction = True
                print("start to search destination")

            if self.config.DESTINATION_SEARCH_FRM.ENABLED:
                is_searching_destination = (
                        sum(self.constraints_check) >= len(self.sub_instructions)
                )

                if is_searching_destination:
                    pose = full_pose[0] if len(full_pose.shape) > 1 else full_pose
                    self.dest_frm.update_position_history(pose)
            if sum(self.constraints_check) < len(self.sub_instructions):
                # print(f"sum(self.constraints_check):{sum(self.constraints_check)},len(self.sub_instructions):{len(self.sub_instructions)}")
                ...
                check = self.constraints_monitor(current_constraint,      
                                                 obs[0],                  
                ...
                if self.config.EVAL.EXECUTE_SOFT_BACKTRACK:

                    if hasattr(self.policy, 'fmm_dist'):
                        current_fmm_dist = np.min(self.policy.fmm_dist)
                    else:
                        current_fmm_dist = 0
                    self.soft_backtrack_controller.record_step(current_fmm_dist)

                    num_sub_instructions = len(self.sub_instructions)
                    should_backtrack, debug_info = self.soft_backtrack_controller.should_backtrack(
                        num_sub_instructions=num_sub_instructions,
                        check_results=check,
                        constraint_steps=constraint_steps
                    )

                    if should_backtrack or float(debug_info.get('total_score', '0')) > 0.4:
                        print(f"\n[BACKTRACK DEBUG] {debug_info}")

                    if should_backtrack:

                        backtrack_info = self.soft_backtrack_controller.execute_soft_backtrack()

                        if backtrack_info['reset_constraint_steps']:
                            constraint_steps = 0

                        if backtrack_info['need_look_around']:
                            full_pose, obs, dones, infos, full_map, one_step_full_map = self._look_around()
                            if dones[0]:
                                infos[0]['reached_last_subinstr'] = self.reached_last_subinstruction
                                infos[0]['locked_target'] = self.policy.locked_target
                                infos[0]['locked_target_step'] = self.policy.locked_target_step
                                infos[0]['sub_instruction_switches'] = self.sub_instruction_switches
                ...
                if start_to_wait and (constraint_steps >= self.min_constraint_steps): 
                    if False in self.constraints_check: 
                        current_idx = self.constraints_check.index(False) 
                        print(f"sub_instr_{current_idx}: {self.sub_instructions[current_idx]}")
                        self.sub_instruction_switches.append({
                            'step': step,
                            'from_idx': current_idx - 1,
                            'to_idx': current_idx,
                            'from_subinstr': self.sub_instructions[current_idx - 1] if current_idx > 0 else 'initial',
                            'to_subinstr': self.sub_instructions[current_idx]
                        })
            ...
            if self.enable_backtrack and self.backtrack_controller is not None:
                policy_output = self._action
                """
                policy_output:
                    {"action": action,
                    "waypoint_candidates": sorted_waypoints,
                    "candidate_scores": sorted_values
                    "best_waypoint": best_waypoint,
                    "best_value": best_value}
                """
                waypoint_candidates = policy_output.get('waypoint_candidates', [])
                candidate_scores = policy_output.get('candidate_scores', [])
                best_waypoint = policy_output.get('best_waypoint', [])
                best_value = policy_output.get('best_value', [])

                if len(waypoint_candidates) > 0 and len(candidate_scores) > 0:
                    candidates_list = list(zip(waypoint_candidates, candidate_scores))
                else:
                    candidates_list = [(best_waypoint, best_value)]
                is_searching_destination = (sum(self.constraints_check) >= len(self.sub_instructions))
                has_locked_target = getattr(self.policy, 'locked_target', False)

                backtrack_result = self.backtrack_controller.step(
                    step=step,
                    pose=full_pose[0],
                    constraint_idx=current_idx,
                    current_constraint=current_constraint,
                    constraint_steps=constraint_steps,
                    max_constraint_steps=self.max_constraint_steps,
                    waypoint_candidates=candidates_list,
                    candidate_scores=candidate_scores,
                    value_map=self.value_map_module.value_map[1],  
                    trajectory_mask=history_map,
                    collided=collided,
                    collision_threshold=30,
                    empty_value_map_count=empty_value_map,
                    is_searching_destination=is_searching_destination,
                    has_locked_target=has_locked_target,
                    current_position=np.array([full_pose[0][1] * 100 / self.resolution,
                                               full_pose[0][0] * 100 / self.resolution])
                )

                if backtrack_result['should_backtrack']:
                    print(f"\n{'=' * 70}")
                    print(f"[BACKTRACK TRIGGERED] Step {step}")
                    print(f"{'=' * 70}")
                    print(f"   Level: {backtrack_result['backtrack_level']}")
                    print(f"   Regret: {backtrack_result['regret_info']['regret']:.3f}")
                    print(f"   New waypoint: {backtrack_result['new_waypoint']}")
                    print(f"{'=' * 70}\n")

                    self.backtrack_waypoint = backtrack_result['new_waypoint']
                    self.is_backtracking = True
                    self.backtrack_start_step = step
                    self.backtrack_controller.set_backtracking(True)
            if self.config.EVAL.EXECUTE_SOFT_BACKTRACK:
                is_searching_destination = (sum(self.constraints_check) >= len(self.sub_instructions))
                has_locked_target = getattr(self.policy, 'locked_target', False)
                should_periodic_backtrack, debug_info = self.soft_backtrack_controller.should_periodic_backtrack(
                    current_step=step,
                    is_searching_destination=is_searching_destination,
                    has_locked_target=has_locked_target
                )

                if debug_info.get('decision') == 'PERIODIC_BACKTRACK':
                    print(f"\n[PERIODIC BACKTRACK DEBUG] {debug_info}")

                if should_periodic_backtrack:

                    if self.config.DESTINATION_SEARCH_FRM.ENABLED:
                    
                        pose = full_pose[0] if len(full_pose.shape) > 1 else full_pose
                        current_y = int(pose[1] * (100 / self.resolution))
                        current_x = int(pose[0] * (100 / self.resolution))
                        current_map_position = np.array([current_y, current_x])

                        self.dest_frm.check_and_mark(
                            current_map_position=current_map_position,
                            backtrack_triggered=True
                        )

                    backtrack_info = self.soft_backtrack_controller.execute_periodic_backtrack(
                        current_step=step
                    )
            ...
            blip_value = self.value_map_module.get_blip_value(Image.fromarray(obs[0]['rgb']),
                                                                self.destination)
            blip_value = blip_value.detach().cpu().numpy()
            value_map = self.value_map_module(step,
                                                full_map[0],  
                                                self.floor,  
                                                self.one_step_floor,  
                                                self.collision_map,  
            ...
            if self.config.DESTINATION_SEARCH_FRM.ENABLED and is_searching_destination and self.policy.locked_target_step == None:
                self.value_map_module.value_map[1] = self.dest_frm.apply_penalty(
                    value_map=self.value_map_module.value_map[1],
                    current_step=step,
            ...
            goal_position = episode.goals[0].position  
            # goal_position = goal_position + 12 - start_position
            self._action = self.policy(self.value_map_module.value_map[1] * history_map, 
                                       self.collision_map,                               
                                       full_map[0],                                      
                                       self.floor,                                       
                                       self.traversible,                                 
                                       ...
                                       step,
                                       goal_position,
                                       backtrack_waypoint=self.backtrack_waypoint,  
                                       is_backtracking=self.is_backtracking  
                                       )
            if self.is_backtracking:
                if self.backtrack_waypoint is not None:
                    current_pos = np.array([full_pose[0][1] * 100 / self.resolution,
                                            full_pose[0][0] * 100 / self.resolution])
                    dist_to_backtrack_wp = np.linalg.norm(current_pos - self.backtrack_waypoint)


                    if dist_to_backtrack_wp < 5 or (step - self.backtrack_start_step) > 10:

                        self.backtrack_waypoint = None
                        self.is_backtracking = False
                        self.backtrack_start_step = None
                        self.backtrack_controller.set_backtracking(False)
                        ...
                        if dones[0]:
                            ...
                            break
        if self.config.EVAL.EXECUTE_SOFT_BACKTRACK:
            stats = self.soft_backtrack_controller.get_statistics()
            print(f"\n[BACKTRACK STATS] Episode {self.current_episode_id}:")
    ...
    def eval(self):
        ...
        for i in tqdm(range(eps_to_eval)):
            self.reached_last_subinstruction = False
            self.rollout()
            self.reset()
